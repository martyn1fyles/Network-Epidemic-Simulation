...
def test_determine_new_infections():
    ...
    test_copy = test_sim_2
    [test_copy.epi_data[node].update({"Resistance": 0.039}) for node in test_copy.node_keys[0:10]]
    [test_copy.epi_data[node].update({"Resistance": 100}) for node in test_copy.node_keys[10:200]]
...
def test_iterate_epidemic_successfully():
    ...
    test_copy = test_sim_2
    [test_copy.epi_data[node].update({"Resistance": 0.039}) for node in test_copy.node_keys[0:10]]
    [test_copy.epi_data[node].update({"Resistance": 1000}) for node in test_copy.node_keys[10:200]]
    test_copy.determine_new_infections()
...
def test_iterate_epidemic_real_epidemic():
    npr.seed(1)
    [test_copy.epi_data[node].update({"Resistance": 0.039}) for node in test_copy.node_keys[0:10]]
    [test_copy.epi_data[node].update({"Resistance": 0.21}) for node in test_copy.node_keys[10:20]]
    test_copy.determine_new_infections()
    test_copy.iterate_epidemic()
    assert test_copy.time > 1
